...
```{r}
tau.19 <- as.numeric(-diff(shootings.dataset$date[2:11])) #Calculating the time between two successive mass shootings in the year 2019

tau.sd.19 <- sd(tau.19) 
tau.mean.19 <- mean(tau.19)

coef.variation.19 <- tau.sd.19/tau.mean.19 # Calculating the coefficient of variation for 2019
...
bootstrap.rep.19 <- replicate(10000, bootstrap.coef.var(365, 10))

hist(bootstrap.rep.19-1, breaks="FD")
abline(v=coef.variation.19-1)
abline(v=-coef.variation.19+1)
```


### Function for the p-value 2019 
```{r}

right.tail.19 <- bootstrap.rep.19-1 >= coef.variation.19-1 # true=values to the right of the hist

left.tail.19 <- bootstrap.rep.19-1 <= -coef.variation.19+1 # true=values to the left of the hist
  
right.tail.19 | left.tail.19 # TRUE where they're in the right tail OR left tail
  
(sum(right.tail.19 | left.tail.19, na.rm=TRUE))/sum(!is.na(bootstrap.rep.19)) #p value 
```
...
tau.sd.15 <- sd(tau.2015) 
tau.mean.15 <- mean(tau.2015)

coef.variation.15 <- tau.sd.15/tau.mean.15 # Calculating the coefficient of variation for 2015

coef.variation.15-1
```

### 2015 Bootstrap Function 
=======
### Centered at 0 
>>>>>>> cfc05e1f15838fa29bffaebbad53de352a2eff23

```{r}
bootstrap.rep.15 <- replicate(10000, bootstrap.coef.var(365, 7))

hist(bootstrap.rep.15-1, breaks="FD")
abline(v=coef.variation.15-1)
abline(v=-coef.variation.15+1)
```
...
### P-value for 2015 
```{r}
right.tail.15 <- bootstrap.rep.15-1 >= coef.variation.15-1 # true=values to the right of the hist

left.tail.15 <- bootstrap.rep.15-1 <= -coef.variation.15+1 # true=values to the left of the hist
  
right.tail.15 | left.tail.15 # TRUE where they're in the right tail OR left tail
  
(sum(right.tail.15 | left.tail.15, na.rm=TRUE))/sum(!is.na(bootstrap.rep.15)) #p value 
```
...
plot(a, d, main = "Mass Shootings Before 9/11/2001",xlab = "Year", ylab = "Total Victims")
plot(x, w, main = "Mass Shootings After 9/11/2001", xlab = "Year", ylab = "Total Victims")
```
...
plot(a, c, main = "Mass Shootings Before 9/11/2001",xlab = "Year", ylab = "Injured")
plot(x, y, main = "Mass Shootings After 9/11/2001", xlab = "Year", ylab = "Injured")
